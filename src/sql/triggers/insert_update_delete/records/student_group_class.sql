CREATE
OR ALTER TRIGGER Trigger_InsertStudentGroupClass ON StudentGroupClass INSTEAD OF
INSERT,
    UPDATE,
    DELETE AS BEGIN
SET NOCOUNT ON;
...
) BEGIN RAISERROR ('StudentGroupClass already exists!', 16, 1);

END -- Check if student hours don't exceed 6 --
ELSE IF (
...
) > (6 * 60) BEGIN RAISERROR ('Student working time exceeds 6!', 15, 0)
END
...
END TRY BEGIN CATCH PRINT 'Error occurred when trying to insert StudentGroupClass!';
