CREATE TRIGGER Trigger_InsertMode ON Mode INSTEAD OF
INSERT AS BEGIN
SET NOCOUNT ON;
DECLARE @name VARCHAR(50)
SELECT @name = name
FROM inserted;
IF @name IS NULL BEGIN RAISERROR ('Mode must not be empty!', 16, 1);
END
ELSE IF @name NOT IN ('regular', 'part') BEGIN RAISERROR (
